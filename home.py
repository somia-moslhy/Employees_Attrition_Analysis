...
from utils import load_data, render_sidebar, DARK, ACCENT, DANGER, GREEN
...
with h2:
    st.image("kayfa_logo.png", width='stretch')
...
rate     = round(df['attrition'].mean() * 100, 1) if total > 0 else 0
avg_inc  = df['monthly_income'].mean() if total > 0 else 0
...
remote_on  = round(df[df['remote_work']=='No']['attrition'].mean()*100, 1) if len(df[df['remote_work']=='No']) > 0 else 0
remote_off = round(df[df['remote_work']=='Yes']['attrition'].mean()*100, 1) if len(df[df['remote_work']=='Yes']) > 0 else 0
promo_0    = round(df[df['number_of_promotions']==0]['attrition'].mean()*100, 1) if len(df[df['number_of_promotions']==0]) > 0 else 0
promo_3    = round(df[df['number_of_promotions']>=3]['attrition'].mean()*100, 1) if len(df[df['number_of_promotions']>=3]) > 0 else 0
single_r   = round(df[df['marital_status']=='Single']['attrition'].mean()*100, 1) if len(df[df['marital_status']=='Single']) > 0 else 0

with d1:
    st.markdown("#### 01 — Expand Remote Work")
    st.markdown(f"On-site employees leave at **{remote_on}%**. Remote employees leave at only **{remote_off}%** — a 28 pp gap, the largest single lever in the data.")
    st.info("Action: Run a remote-eligibility pilot for feasible roles next quarter.")

with d2:
    st.markdown("#### 02 — Fix the Promotion Path")
    st.markdown(f"Employees with 0 promotions leave at **{promo_0}%**. Three or more drops that to **{promo_3}%**. The first two promotions alone do not retain.")
...
with d3:
    ...
    st.markdown(f"Single employees leave at **{single_r}%** — nearly double married staff. They are mobile and need a reason to stay.")
...
with c2:
    drivers_df = pd.DataFrame({
        ...
        "Impact": [
            round(df[df['remote_work']=='No']['attrition'].mean()*100 - df[df['remote_work']=='Yes']['attrition'].mean()*100, 1) if len(df[df['remote_work']=='Yes']) > 0 else 0,
            round(df[df['work_life_balance']=='Poor']['attrition'].mean()*100 - df[df['work_life_balance']=='Excellent']['attrition'].mean()*100, 1) if len(df[df['work_life_balance']=='Poor']) > 0 else 0,
            round(df[df['number_of_promotions']==0]['attrition'].mean()*100 - df[df['number_of_promotions']>=3]['attrition'].mean()*100, 1) if len(df[df['number_of_promotions']>=3]) > 0 else 0,
            round(df[df['overtime']=='Yes']['attrition'].mean()*100 - df[df['overtime']=='No']['attrition'].mean()*100, 1) if len(df[df['overtime']=='Yes']) > 0 else 0,
            round(df[df['leadership_opportunities']=='No']['attrition'].mean()*100 - df[df['leadership_opportunities']=='Yes']['attrition'].mean()*100, 1) if len(df[df['leadership_opportunities']=='No']) > 0 else 0,
        ]
    ...
    fig = px.bar(drivers_df, x="Impact", y="Driver", orientation="h",
                 text="Impact",
                 color="Impact",
                 color_continuous_scale=[[0, ACCENT],[1, DANGER]],
